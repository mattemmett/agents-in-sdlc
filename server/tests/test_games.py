...
class TestGamesRoutes(unittest.TestCase):
    ...
    def test_create_game_success(self) -> None:
        ...
        new_game_data = {
            ...
            "description": "Navigate the treacherous waters of code reviews",
    ...
    def test_create_game_invalid_title(self) -> None:
        ...
        new_game_data = {
            "title": "A",
            "description": "A game with invalid title",
            "category_id": category.id,
            "publisher_id": publisher.id,
            "star_rating": 4.0
        }
    ...
    def test_create_game_invalid_description(self) -> None:
        """Test game creation with invalid description (too short)"""
        # Arrange
        with self.app.app_context():
            publisher = db.session.query(Publisher).first()
            category = db.session.query(Category).first()
    ...
    def test_update_game_invalid_title(self) -> None:
        """Test update with invalid title (too short)"""
        # Arrange - Get an existing game
        response = self.client.get(self.GAMES_API_PATH)
        games = self._get_response_data(response)
        game_id = games[0]['id']
    ...
    def test_update_game_invalid_description(self) -> None:
        """Test update with invalid description (too short)"""
        # Arrange - Get an existing game
        response = self.client.get(self.GAMES_API_PATH)
        games = self._get_response_data(response)
        game_id = games[0]['id']
